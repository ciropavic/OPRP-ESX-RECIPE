...
Citizen.CreateThread(function()
    while true do
        if localTime == 1 then -- client time
            ...
            drawTimeText = featColor .. "~r~Time: ~b~" .. hour .. ":" .. newMinute .. featColor .."~r~ | Date: ~b~" .. day .. featColor .."~r~/~b~" .. month .. featColor .. "~r~/~b~" .. year
        ...
        if peacetimeActive then
            ...
            DrawTextAOP(AOPxNew, AOPyNew, 1.0,1.0,0.40, drawTimeText, 255, 255, 255, 255)
            DrawTextAOP(AOPxNew, AOPyNew2, 1.0,1.0,0.40, "~w~Current " .. featColor .. "AOP: ~w~" .. FaxCurAOP .. featColor .. " | ~w~PeaceTime: ~g~Enabled", 255, 255, 255, 255)
        elseif not peacetimeActive then
            if peacetime then
                DrawTextAOP(AOPxNew, AOPyNew, 1.0,1.0,0.40, drawTimeText, 255, 255, 255, 255)
                DrawTextAOP(AOPxNew, AOPyNew2, 1.0,1.0,0.40, "~w~Current " .. featColor .. "AOP: ~w~" .. FaxCurAOP .. featColor .. " | ~w~PeaceTime: ~r~Disabled", 255, 255, 255, 255)
            else
                DrawTextAOP(AOPxNew, AOPyNew, 1.0,1.0,0.40, drawTimeText, 255, 255, 255, 255)
                DrawTextAOP(AOPxNew, AOPyNew2, 1.0,1.0,0.40, "~r~Current " .. featColor .. "~r~AOP: ~b~" .. FaxCurAOP, 255, 255, 255, 255)
            end
        end
	end
end)
