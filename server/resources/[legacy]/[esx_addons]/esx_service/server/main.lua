...
RegisterServerEvent('esx_service:messageAllInService')
AddEventHandler('esx_service:messageAllInService', function(message, name)
	for k,v in pairs(InService[name]) do
		if v == true then
			TriggerClientEvent('esx_service:messageAllInService', k, message, source)
		end
	end
end)
